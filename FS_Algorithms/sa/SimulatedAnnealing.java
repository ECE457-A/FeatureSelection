...
//        startSA(solution, 0.8, 0.1);
        startSA(solution, 0.02, 0.1);
        
	}
	
	public static void startSA(int[] solution, double temp, double alpha) {
		int maxIterationsPerTemp = 10;
		double goalAccuracy = 0.9;
		...
		double toleranceRate = 0.4;
		
		while (currentAccuracy < goalAccuracy) {
			for (int i = 0; i < maxIterationsPerTemp; i++) {
				potentialSolution = flipBits(solution, toleranceRate);
			...
			System.out.println("NOW CHANGING TEMPERATURE");
			temp *= alpha;
		}
	...
	public static int[] flipBits(int[] solution, double toleranceRate) {
		int[] result = new int[solution.length];
//		
//		for (int i = 0; i < result.length; i++) {
//			if (ThreadLocalRandom.current().nextDouble(0, 1) > toleranceRate) {
//				if (solution[i] == 1)
//					result[i] = 0;
//				else
//					result[i] = 1;
